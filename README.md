...
* So i want to apply some techniques that minimize the frequency or energy displayed in the frequency bands that don't represent SCG heart beat complexes.. I'm going to use a type of mode decompisition that is inspired by successive variational mode decomposition (SVMD), although in practice the functions aren't a strict implementation of SVMD, that you can apply to a signal so that the signal is decomposed into a set of signals that represent the original signal, these modes will try to capture the periodic "oscillations" which are representing the heart beat complexes in the signal. This decomposition should also in theory not capture the noise in the signal especially this background diffuse noise present throughout the signal.
...
### Mode Decomposition Visualization 

* The first main step in the method is to apply the mode decomposition to our SCG signal to try to better extract our Heart beat locations. This is important because our actual peak detection method will require clean peaks to accurately identify real heart beats. 
